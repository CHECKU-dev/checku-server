...
EXIST_A=$(sudo docker-compose -p checku-a -f docker-compose.a.yml ps | grep Up)
...
sudo docker-compose -p checku-${START_CONTAINER} -f docker-compose.${START_CONTAINER}.yml up -d --build
...
echo "checku-${TERMINATE_CONTAINER} down"
sudo docker-compose -p checku-${TERMINATE_CONTAINER} -f docker-compose.${TERMINATE_CONTAINER}.yml down
